...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        message.reply({ embeds: [embed.setDescription(`${member} kullanıcısı ${author} tarafından karantinaya atıldı. \`(Ceza ID: #${db.fetch(`ceza_${guild.id}`)})\``)] }).catch((err) => console.log(err), client.ytick(message))
    ...
    db.set(`${cezaID}`, `${author} tarafından ${moment(Date.now()).format("LLL")} tarihinde **[KARANTİNA]** cezası almış.`)
